...
GOLD_EDIT_INTERVAL = 1      # هر چند ثانیه، پیام قیمت طلا رو ویرایش کنه
...
def get_gold_price():
    """
    قیمت طلای اسپات (نقطه‌ای) رو از طریق PAXG می‌گیره — یه توکن دیجیتال که
    پشتوانه‌ش طلای فیزیکی واقعیه (هر واحد = ۱ اونس طلا تو یه خزانه) و قیمتش
    تقریباً همیشه چسبیده به قیمت جهانی اسپات طلا. چون دارایی کریپتوییه،
    یاهو فایننس ۲۴/۷ و بدون مشکل ارائه‌ش می‌ده (برخلاف نماد فارکسی XAUUSD=X
    که به‌طور پایدار جواب نمی‌داد). اگه این هم در دسترس نبود، به فیوچرز
    کوموکس سوییچ می‌کنه که همیشه در دسترسه ولی چند ده دلار بالاتره.
    """
    price = fetch_price("PAXG-USD")
    if price is not None:
        return price

    logging.warning("قیمت PAXG در دسترس نبود؛ از فیوچرز طلا به‌عنوان جایگزین استفاده می‌شه")
